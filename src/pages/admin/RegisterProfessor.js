...
export default function RegisterProfessor() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post("/api/user/professor", formData);
      alert("교수가 등록되었습니다.");
      navigate("/staff/professor-list");
    } catch (err) {
      console.error("교수 등록 실패:", err);
      if (err.response?.data?.message) {
        setError(err.response.data.message);
      } else {
        setError("교수 등록에 실패했습니다.");
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="my-page-container">
      <aside className="side-menu">
        ...
        <nav className="side-menu-nav">
          ...
          <Link to="/staff/register-professor" className="menu-item active">
            교수 등록
          </Link>
        ...
        <form onSubmit={handleSubmit} className="register-form">
          <table className="form-table">
            <tbody>
              ...
              <tr>
                <th>
                  <label htmlFor="deptId">학과 ID</label>
                </th>
                <td>
                  <input
                    type="number"
